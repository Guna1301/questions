...
class p2{
    public static void main(String args[]){
        ...
        System.out.println(find(dict,words));
        sc.close();
    }
    public static List<String> find(String dict[], String words[] ){
        Set<String> exact = new HashSet<>();
        Map<String,String> insensitive = new HashMap<>();
        Map<String,String> vowelStrings = new HashMap<>();
        for(String dword:dict){
            exact.add(dword);
            String lowerS = dword.toLowerCase();
            insensitive.put(lowerS,dword);
            String vowelS = toVowel(lowerS);
            vowelStrings.put(vowelS,dword);
        }
        ...
        for(String word:words){
            if(exact.contains(word)){
                ...
                if(insensitive.containsKey(lower) ){
                    ans.add(insensitive.get(lower));
                }else{
                    String vowel = toVowel(lower);
                    if(vowelStrings.containsKey(vowel)){
                        ans.add(vowelStrings.get(vowel));
                    }else{
                        ans.add("");
                    }
                }
