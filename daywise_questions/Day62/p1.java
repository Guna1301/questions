...
class Solution{
    public static void main(String args[]){
        ...
        System.out.println(find(0,0,t1,t2,dp));
    }
    public static int find(int id1,int id2, int t1[], int t2[],int dp[][]){
        if(id1>= t1.length || id2>=t2.length){
            return 0;
        }
        if(dp[id1][id2]!=-1){
            return dp[id1][id2];
        ...
        int ans = 0;
        for(int i=id1;i<t1.length;i++){
            int count = 0;
            boolean possible = false;
            int j = id2;
            for(j=id2;j<t2.length;j++){
                if(t1[i]==t2[j]){
                    possible = true;
                    break;
                }
