...
public class Solution{
    public static void main(String args[]){
        ...
        System.out.println(find(items,rooms));
    }
    public static int find(int items[], int rooms[]){
        Arrays.sort(items);
        int filled[] = new int[rooms.length];
        int ans = 0;
        filled[0] = rooms[0];
        ...
        int j=0;
        for(int i=rooms.length-1;i>=0 && j<items.length;i--){
            if(filled[i]>=items[j]){
                j++;
                ans++;
            }
