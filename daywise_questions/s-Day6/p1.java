...
class p1{
    public static void main(String args[]){
        ...
        int s = sc.nextInt();
        System.out.println(find(s1,s2,s));
        sc.close();
    }
    public static boolean find(String s1, String s2, int s){
        Map<Integer,Integer> stepFreq = new HashMap<>();
        for(int i=0;i<s1.length();i++){
            ...
            stepFreq.put(steps,stepFreq.getOrDefault(steps,0)+1);
        }
        for(Map.Entry<Integer,Integer> entry:stepFreq.entrySet()){
            int steps = entry.getKey();
            int freq = entry.getValue();
            int max = steps+26*(freq-1);
            if(max>s){
                return false;
            }
        }
