...
class p3{
    public static void main(String args[]){
        ...
        System.out.println(find(arr,n,ma,mf));
    }
    // public static int find(int arr[], int n, int ma,int mf){
    //     int ans = 0;
    //     for(int k=1;k<=n;k++){
    //         int sum = 0;
    ...
    //     return ans;
    // }
    // public static boolean valid(Map<Integer,Integer> freq,int mf){
    ...
    public static int find(int arr[], int n, int ma,int mf){
        ...
        for(int right = 0; right < n; right++){
            ...
            if(freq.get(arr[right])>mf){
                invalid++;
            }
            while(invalid>0 || sum>ma){
                if(freq.get(arr[left])>mf){
                    invalid--;
                }
                sum-=arr[left];
