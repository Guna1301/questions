...
class p1{
    public static void main(String args[]){
        ...
        System.out.println(find(arr,1,n,dp));
    }
    public static int find(int arr[],int left,int right,int dp[][]){
        if(left>right){
            return 0;
        }
        if(dp[left][right]!=-1){
        ...
        for(int id=left;id<=right;id++){
            ...
            int val = arr[left-1]*arr[id]*arr[right+1];
            int res = val+find(arr,left,id-1,dp)+find(arr,id+1,right,dp);
