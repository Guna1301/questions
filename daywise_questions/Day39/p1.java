...
class p1{
    public static void main(String args[]){
        ...
        for(int i=0;i<3;i++){
            ans = Math.min(ans,arr[n-1][i]+find(n-2,i,arr,dp));
        }
        System.out.println(ans);
    }
    public static int find(int id,int color,int arr[][],int dp[][]){
        if(id<0){
            return 0;
        }
        if(dp[id][color]!=-1){
        ...
        for(int i=0;i<3;i++){
            int val = 0;
            if(i==color)continue;
            val += arr[id][i]+find(id-1,i,arr,dp);
            total = Math.min(total,val);
        }
        return dp[id][color] = total;
    }
