...
public class p2 {
    ...
    static int findx(HashMap<Integer, Integer> freq, int f, int x){
        PriorityQueue<pair> q = new PriorityQueue<>((a,b)->b.val==a.val?Integer.compare(b.key, a.key):Integer.compare(b.val, a.val));
        for(Map.Entry<Integer, Integer> e : freq.entrySet()){
            int key = e.getKey();
            int val = e.getValue();
            val = val * (int) Math.pow(key, f);
            q.offer(new pair(key, val));
        }
        int sum = 0;
        for(int i=0; i<x; i++){
            pair p = q.poll();
            System.out.println(p.key+" "+p.val+ " "+ freq.get(p.key));
            sum += freq.get(p.key)*p.key;
