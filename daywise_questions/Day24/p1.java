 ...
 */
 ...
 class robosignalsequence{
     public static void main(String args[]){
         ...
         System.out.println(find(bins));
     }
     public static boolean find(String bins[]){
         int n = bins.length;
         for(int i=0;i<n;i++){
             
             if(bins[i].charAt(0)=='1'){
                 int count = 0;
                 int k = 0;
                 while(bins[i].charAt(k)=='1'){
                     count++;
                     k++;
                 }
                 if(count>n-i+1 || count == 0){
                     return false;
                 }
                 int j = i+1;
                 while(count>1){
                     if(!bins[j].startsWith("10")){
                         System.out.println("adsifg");
                         return false;
                     }
                     count--;
                     j++;
