...
class p3 {
    ...
    public static int find(int id, String s, Map<Integer, Integer> dp) {
        if (id == s.length()) {
            return 1;
        }
        ...
        total += find(id + 1, s, dp);

        if (id < s.length() - 1) {
            int twoDigit = Integer.parseInt(s.substring(id, id + 2));
            if (twoDigit >= 10 && twoDigit <= 26) {
                total += find(id + 2, s, dp);
            }
        }
