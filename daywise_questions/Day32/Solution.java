...
class SegmentTree {
    ...
    // Build the segment tree, marking all days as available (1)
    private void build(int node, int start, int end) {
        if (start == end) {
            tree[node] = 1; // Each day is initially free
            return;
        }
        int mid = (start + end) / 2;
        build(2 * node, start, mid);
        build(2 * node + 1, mid + 1, end);
        tree[node] = tree[2 * node] + tree[2 * node + 1]; // Sum of availability
    }

    // Find the first available day in range [L, R]
    public int query(int node, int start, int end, int L, int R) {
        if (start > R || end < L) return -1; // Out of range
        if (start == end) return tree[node] == 1 ? start : -1; // Found available day

        int mid = (start + end) / 2;
        int left = query(2 * node, start, mid, L, R);
        if (left != -1) return left; // If found in left subtree, return immediately
        return query(2 * node + 1, mid + 1, end, L, R); // Check right subtree
    }

    // Mark a day as occupied
    public void update(int node, int start, int end, int day) {
        if (start == end) {
            tree[node] = 0; // Mark as occupied
            return;
        }
        int mid = (start + end) / 2;
        ...
        tree[node] = tree[2 * node] + tree[2 * node + 1]; // Update parent node
    }
...
public class p3 {
    ...
    public int maxEvents(int[][] events) {
        ...
        int maxDay = 0;
        for (int[] event : events) maxDay = Math.max(maxDay, event[1]); // Find max end time
